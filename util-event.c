                     ...
                     const char *default_port)
{
  ...
  if ((cp = strchr(a, ':'))) {
    ...
    *cp = '\0';
    ...
    do_log(L_INFO, "error in address %s: port required", address);
    free(a);
    return NULL;
  }
  ...
  if (ai_res) {
    if (ai_res == EVUTIL_EAI_SYSTEM)
      do_log(L_WARN,"error resolving %s: %s [%s]",
               address, evutil_gai_strerror(ai_res), strerror(ai_errno));
    else
      do_log(L_WARN,"error resolving %s: %s", address, evutil_gai_strerror(ai_res));
    ...
    do_log(L_WARN,"address resolution failed for %s", address);
  }
